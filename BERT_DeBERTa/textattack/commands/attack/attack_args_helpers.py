...
def parse_model_from_args(args):
    if args.model_from_file:
        ...
        model_train_args['device'] = args.device
        config_class, model_class, tokenizer_class = MODEL_CLASSES[model_train_args['model'].split('_')[0]]
        if model_train_args['model'] in ["bert", "deberta"]:
            model = model_class.from_pretrained(model_train_args['output_dir'])
        else:
            with open(os.path.join(model_train_args['output_dir'], 'maskbert.pt'), 'rb') as f:
                model = torch.load(f)
            
        tokenizer = tokenizer_class.from_pretrained(model_train_args['output_dir'], do_lower_case=model_train_args['do_lower_case'])
        model.to(args.device)
        model_wrapper = textattack.models.wrappers.PyTorchModelWrapper(model, tokenizer, model_train_args['train_batch_size'])
...
def dataset_from_my_file(args):
    ...
    mydata_dir = os.path.join(
            current_dir, os.pardir, os.pardir, os.pardir, "textattack", "my_datasets", args.dataset
        )

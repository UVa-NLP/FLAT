...
class PyTorchModelWrapper(ModelWrapper):
    ...
    def __call__(self, text_input_list):
        ...
        # padding
        seq_len_map = {'sst2': 50, 'imdb': 250, 'ag': 50, 'trec': 15}
        try:
            max_seq_len = seq_len_map[self.model.config.finetuning_task]
        except:
            max_seq_len = seq_len_map[self.model.bertmodel.config.finetuning_task]
        try:
            if self.model.args.model_type == 'bert':
                ids = [[101] + ii[:(max_seq_len - 2)] + [102] + [0]*(max_seq_len - len(ii[:(max_seq_len - 2)]) - 2)  for ii in ids]
            else:
                ids = [[1] + ii[1:-1][:(max_seq_len - 2)] + [2] + [0]*(max_seq_len - len(ii[1:-1][:(max_seq_len - 2)]) - 2)  for ii in ids]
        except:
